            ...
            <Paper className={styles.contactPaper} elevation={3}>
                ...
                <form className={styles.contactForm} onSubmit={submitMessage}>
                    <div className={styles.contactTextContainer}>
                        <TextField onChange={handleChange} className={styles.contactText} name='message' label='Sõnum...' variant="outlined" multiline rows={4} required/>                
                    </div>
                    <div className={styles.contactFooter}>
                        <Button disabled={loading || success} className={styles.button} type='submit' variant='contained' color='primary'>{loading && <CircularProgress size={24} className={styles.buttonProgress}/>}{success && <DoneIcon color="primary" className={styles.success}/>}Saada</Button>
                        <Button className={styles.button} onClick={cancel} variant='contained' color='secondary'>Tühista</Button>
                    </div>
                </form>
            </Paper>
        </div>
